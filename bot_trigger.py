...
def run_bot(
    ...
    return asyncio.run(
        trigger_bot(
            bot_name, instruction, repo_name, pr_number, comment_id, comment_type
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bot_name", required=True, choices=["review", "fix"], help="Bot to trigger"
    )
    parser.add_argument(
        "--instruction", required=True, help="Custom instruction for the bot"
    )
